...
version: alpha
name: Clay
description: A vibrant claymation-meets-data interface for Clay.com (GTM data-orchestration platform). Anchors on canvas branco with dark-navy CTA principals, custom rounded display type, and saturated single-color feature cards — hot pink, deep teal, lavender, peach, ochre — that punctuate long-scroll explainer pages. Brand voltage comes from 3D-rendered claymation illustrations (mountains, characters, mascots) used as full-bleed hero artifacts and the bright multi-color card surfaces showing interface do produto fragments.
...
Component voltage comes from **saturated single-color feature cards** in a 6-color palette: hot pink, deep teal, lavender, peach, ochre, and cream-card. Each card shows interface do produto fragments at small scale — Claygent agent runs, sequencer flows, CRM enrichment outputs. The colored card IS the primary visual element on every long-scroll page.

**Key Characteristics:**
- Cream-tinted canvas branco (`{colors.canvas}` — #fffaf0). The warmth differentiates Clay from cool-gray competitor sites.
- Dark navy/black CTA principals (`{colors.primary}` — #0a0a0a). Buttons rounded `{rounded.md}` (12px) — friendly modern but not pill.
...
- Border radius is generous: `{rounded.md}` (12px) for botões + inputs, `{rounded.lg}` (16px) for content cards, `{rounded.xl}` (24px) for feature cards. The bigger radius matches the rounded display type's character.
...
- Footer is cream-tinted (`{colors.surface-soft}`) — Clay does NOT use a dark rodapé. Even the closing band stays warm-light.

## Cores

### Marca e Destaques
- **Primária** (`{colors.primary}` — #0a0a0a): All CTA principals, h1/h2 ink type. Near-black with slight warmth.
...
- **Superfície Soft** (`{colors.surface-soft}` — #faf5e8): Footer and CTA-band background.
- **Superfície Card** (`{colors.surface-card}` — #f5f0e0): Cream feature cards, testimonial cards.
- **Superfície Strong** (`{colors.surface-strong}` — #ebe6d6): Stronger cream for emphasized bands.
- **Superfície Dark** (`{colors.surface-dark}` — #0a1a1a): Dark teal-tinted near-black for occasional dark cards (rare).
- **Superfície Dark Elevated** (`{colors.surface-dark-elevated}` — #1a2a2a): Elevated dark cards.
- **Hairline** (`{colors.hairline}` — #e5e5e5): 1px borders on cards and inputs.

### Textoo
- **Ink** (`{colors.ink}` — #0a0a0a): Headlines and primary text.
- **Corpo Strong** (`{colors.body-strong}` — #1a1a1a): Emphasized body, lead paragraphs.
- **Corpo** (`{colors.body}` — #3a3a3a): Default running-text.
- **Suave** (`{colors.muted}` — #6a6a6a): Sub-headings, breadcrumbs, rodapé body.
- **Suave Soft** (`{colors.muted-soft}` — #9a9a9a): Captions, fine-print.
- **On Primary / On Dark** (`{colors.on-primary}` — #ffffff): Text on primary botões + dark feature cards (teal).
...
The body-vs-display split is functional: Plain Black for Plain Black moments (headlines), Inter for everything else (running text, UI, botões). Mixing them is a system violation.
...
| `{rounded.sm}` | 8px | Small botões, hairline-border accent |
| `{rounded.md}` | 12px | Standard CTA botões, text inputs |
...
**`feature-card-pink`** / **`feature-card-teal`** / **`feature-card-lavender`** / **`feature-card-peach`** / **`feature-card-ochre`** — Saturated single-color feature cards. Background varies per variant; rounded `{rounded.xl}` (24px); padding `{spacing.xl}` (32px). Each card carries an h3 in `{typography.title-md}`, a body description, and a interface do produto fragment or mascot illustration. Text color flips to `{colors.on-dark}` (white) on pink and teal cards, `{colors.ink}` (dark) on lavender/peach/ochre cards (the lighter saturations have enough contrast for dark text).
...
**`product-mockup-card`** — Card showing actual Clay interface do produto (Claygent agent runs, sequencer flows, CRM enrichment tables). Background `{colors.canvas}` with hairline border, rounded `{rounded.lg}`, padding `{spacing.lg}` (24px).
...
**`cta-band-illustrated`** — Pre-rodapé "Turn your growth ideas into reality today" band. Background `{colors.surface-soft}`, rounded `{rounded.xl}`, padding 80px. Carries an h2 in `{typography.display-md}`, a sub-line, and a `{component.button-primary}` — usually paired with a 3D illustration of a mascot or scene.

**`rodapé`** — Cream-tinted rodapé (NOT dark navy unlike most SaaS sites). Background `{colors.surface-soft}`, text `{colors.body}`. 4-column link list. Vertical padding 80px. Often features a horizon-style 3D mountain illustration at the very bottom — Clay's signature rodapé mountain.
...
- Show interface do produto fragments inside saturated feature cards. The brand voltage is product-driven, not abstract.
- Use cream rodapé (NOT dark). Clay deliberately closes pages with warm cream rather than the standard dark-rodapé SaaS template.
...
- Don't use a dark rodapé. The cream rodapé is part of the system's warm-throughout pacing.
...
7. The cream-throughout palette is a system contract — don't add a dark rodapé.
