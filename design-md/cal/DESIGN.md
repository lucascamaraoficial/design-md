...
version: alpha
name: Cal.com
description: A clean, calendar-software-first interface anchored on canvas branco with black CTA principals and custom Cal Sans display typography. The system reads as SaaS moderno e amigável — generous whitespace, soft-rounded cards (~12px), interface do produto fragments shown directly inside cards, and a dark navy rodapé that visually closes long-scroll pages. Brand voltage comes from the Cal Sans display headline (a custom geometric face) and from interface do produto artifacts shown in-card rather than from accent colors.
...
Cal.com's marketing surface is a clean, friendly modern-SaaS interface — canvas branco (`{colors.canvas}` — #ffffff) with black CTA principals (`{colors.primary}` — #111111), custom **Cal Sans** display typography, and `{colors.surface-card}` (#f5f5f5) light-gray cards holding interface do produto fragments. The system reads as confidently engineered without trying to impress — every band has clear hierarchy, generous whitespace, and a single primary action.

Type voice splits cleanly into two roles: **Cal Sans** (the brand's custom geometric display face — used for h1, h2, h3, and hero headlines) and **Inter** (used for everything else — body, botões, nav, captions). Cal Sans uses weight 600 with negative letter-spacing (-0.5px to -2px depending on size) — it feels modern, slightly condensed, distinctly Cal.com.

Component voltage comes from **interface do produto fragments shown directly inside cards** — calendar widgets, scheduling forms, automation diagrams, integration tiles. Cal.com doesn't paint marketing illustrations of the product; it shows the actual product chrome at small scale embedded in the marketing flow.

The rodapé flips to `{colors.surface-dark}` (#101010) — a deep near-black that visually closes every long-scroll page. The rodapé is the only superfície escura in the system; everything above stays white-with-light-gray-cards.

**Key Characteristics:**
- White canvas with black CTA principal (`{colors.primary}` — #111111). Buttons are `{rounded.md}` (8px) with confident weight-600 labels. Standard friendly-SaaS button.
...
- Footer is dark navy (`{colors.surface-dark}` — #101010) with light text (`{colors.on-dark-soft}` — #a1a1aa). The dark rodapé closes every page even though the body above is white.
...
- Border radius is hierarchical: `{rounded.md}` (8px) for botões + inputs, `{rounded.lg}` (12px) for content cards, `{rounded.xl}` (16px) for the hero app-mockup container, `{rounded.pill}` for nav-pill-group + badges, `{rounded.full}` for avatars + icon botões.

## Cores

### Marca e Destaques
- **Primária** (`{colors.primary}` — #111111): The dominant action color. All CTA principals, h1/h2 display type. Press state shifts to `{colors.primary-active}` (#242424).
...
- **Badge Pastels** — A small pastel set for category badges and avatar fills: `{colors.badge-orange}` (#fb923c), `{colors.badge-pink}` (#ec4899), `{colors.badge-violet}` (#8b5cf6), `{colors.badge-emerald}` (#34d399). These appear on tag pills and small accent moments inside interface do produto fragments — never on hero CTAs.

### Superfície
- **Canvas** (`{colors.canvas}` — #ffffff): The default page floor.
- **Superfície Soft** (`{colors.surface-soft}` — #f8f9fa): Nav-pill-group background, very-soft section dividers.
- **Superfície Card** (`{colors.surface-card}` — #f5f5f5): Feature cards, testimonial cards, badge pills, default avatar fills.
- **Superfície Strong** (`{colors.surface-strong}` — #e5e7eb): Hairline border alternative; disabled button background.
- **Superfície Dark** (`{colors.surface-dark}` — #101010): The rodapé background — the only superfície escura on every page. Also used for the featured pricing tier card.
- **Superfície Dark Elevated** (`{colors.surface-dark-elevated}` — #1a1a1a): Used for nested cards inside the dark rodapé or featured pricing card.
...
- **Hairline Soft** (`{colors.hairline-soft}` — #f3f4f6): A barely-visible divider used between sections that share the canvas branco.

### Textoo
- **Ink** (`{colors.ink}` — #111111): All headlines and primary text.
- **Corpo** (`{colors.body}` — #374151): Default running-text color.
- **Suave** (`{colors.muted}` — #6b7280): Secondary text — sub-headings, breadcrumbs, rodapé body.
- **Suave Soft** (`{colors.muted-soft}` — #898989): Tertiary text — captions, fine-print, copyright lines.
- **On Primary / On Dark** (`{colors.on-primary}` / `{colors.on-dark}` — #ffffff): Text on primary botões and dark rodapé.
- **On Dark Soft** (`{colors.on-dark-soft}` — #a1a1aa): Footer body text — slightly muted white for the link rows.

### Semantic
- **Success** (`{colors.success}` — #10b981): Confirmation states, success badges in interface do produto.
...
### Font Family
The system runs **Cal Sans** for display + brand wordmark and **Inter** for everything else. Cal Sans is Cal.com's custom geometric display typeface — slightly condensed, weight 600, negative letter-spacing. Inter handles body, botões, navigation, captions, and tabular code blocks. The fallback stack walks `-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif` for both families.

The split is functional:
- Cal Sans (display, 600 weight, -0.5 to -2px tracking) — h1, h2, h3
- Inter (body + UI, 400-600 weight, 0 letter-spacing) — paragraphs, labels, botões, nav
...
- **Gutters:** `{spacing.lg}` (24px) between cards in 3-up grids; `{spacing.md}` (16px) inside rodapé columns.
...
- **Rodapé:** 4-column link list at desktop, wrapping to 2-up at tablet, 1-up at mobile.
...
| Featured tier | `{colors.surface-dark}` background, no shadow needed | The featured pricing tier inverts to superfície escura — color contrast does the elevation work |
...
### Decorative Depth
- Calendar widgets and interface do produto fragments embedded inside marketing cards carry their own internal shadows from the interface do produto itself — these are not system tokens, they're product chrome shown as content.
...
| `{rounded.sm}` | 6px | Small inline botões, dropdown items |
| `{rounded.md}` | 8px | Standard CTA botões, text inputs, category tabs |
...
| `{rounded.full}` | 9999px / 50% | Avatars, icon botões |
...
**`button-primary`** — The signature CTA principal. Background `{colors.primary}` (#111111), text `{colors.on-primary}`, type `{typography.button}` (Inter 14px / 600), padding 12px × 20px, height 40px, rounded `{rounded.md}` (8px). Active state `button-primary-active` shifts to `{colors.primary-active}` (#242424).
...
**`product-mockup-card`** — A card showing actual Cal.com interface do produto fragments (workflow editor, calendar grid, integration grid, automation flow). Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.lg}` (24px). The interface do produto inside has its own internal chrome — these cards display the product, they don't decorate around it.
...
**`pricing-tier-card-featured`** — The featured tier (typically "Teams"). Background flips to `{colors.surface-dark}` (#101010), text inverts to `{colors.on-dark}`. The superfície escura IS the featured-tier signal — no accent border, no badge, no scale shift.
...
**`cta-band-light`** — A pre-rodapé "Smarter, simpler scheduling" CTA card. Background `{colors.surface-card}`, rounded `{rounded.lg}`, padding `{spacing.xxl}` (48px). Carries an h2 in `{typography.display-sm}`, a sub-line, and a `{component.button-primary}` centered.

**`rodapé`** — Dark navy rodapé that closes every page. Background `{colors.surface-dark}` (#101010), text `{colors.on-dark-soft}`. 4-column link list at desktop covering Product / Solutions / Company / Resources. Vertical padding 64px. The Cal.com wordmark sits at the top-left in `{colors.on-dark}`. The rodapé is the only superfície escura on every page — the deliberate inversion visually closes the page.

## Do's and Don'ts

### Do
- Reserve `{colors.primary}` (#111111) for CTA principals and h1/h2 type. Cal.com's button is near-black, not blue.
...
- Embed real interface do produto fragments inside marketing cards. Don't paint marketing illustrations of the product when you can show the product itself.
...
- End every page with the dark rodapé. The light-to-dark transition is part of the editorial rhythm.

### Don't
- Don't use accent colors (`{colors.brand-accent}`, badge pastels) on CTA principals. The system is monochrome at the action layer.
...
- Don't put superfície escura cards anywhere except the rodapé and the featured pricing tier. The superfície escura is a deliberate, scarce signal.
- Don't repeat the same surface mode in two consecutive bands. Cal.com's pacing alternates white → light-gray → white → product-mockup-card → white → dark-rodapé.
...
|---|---|---|
| Mobile | < 768px | Hamburger nav; hero h1 64→32px; hero-app-mockup-card stacks below content; feature grids 1-up; pricing 1-up; rodapé 4 cols → 1 |
...
- Hero band's 7-5 grid collapses to single-column on mobile — h1 + sub-head + botões first, then the app-mockup card below.
- Feature grids reduce columns rather than scaling cards down.
- Pricing tier cards collapse 4 → 2 → 1; featured-tier superfície escura stays visually distinct at every breakpoint.
...
6. The dark rodapé is the only superfície escura on most pages. Don't add other dark cards casually.
