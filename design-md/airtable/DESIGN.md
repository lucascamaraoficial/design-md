...
version: alpha
name: Airtable
description: A sober, editorial workflow-software interface anchored on canvas branco and dark-ink type, where brand voltage comes from full-bleed signature cards in coral, dark green, peach, and dark navy that punctuate long-scroll explainer pages. Primary actions use a near-black pill CTA; secondary actions sit in a white outlined button. Type runs Haas Grotesk in modest weights — never bold for its own sake.
...
- Hero is canvas branco. There is no atmospheric gradient, no mesh, no background flourish. The brand strength comes from the type and the botões sitting in clean whitespace.
...
- Demo-card grids carry interface do produto fragments on `{colors.signature-peach}`, `{colors.signature-mint}`, `{colors.signature-cream}` and other warm pastel surfaces.
- Section rhythm: canvas branco → coral signature card → white body → cream callout band → dark navy CTA → light gray CTA banner → rodapé. The canvas resets between every signature surface.
- Border radius is hierarchical: `{rounded.lg}` (12px) for CTA principals and large signature cards, `{rounded.md}` (10px) for content cards and demo grids, `{rounded.sm}` (6px) for inputs, `{rounded.full}` for icon botões. Pricing botões jump to `{rounded.pill}` to mark themselves as a separate dialect.
- Vertical rhythm is `{spacing.section}` (96px) between major bands — universal across every page.

## Cores

### Marca e Destaques
- **Primária** (`{colors.primary}` — #181d26): The dominant brand color. Used for the CTA principal background, h1/h2 display type, and the `{component.surface-dark}` band. Not "blue, then black" — black IS the primary throughout the marketing system.
- **Primária Active** (`{colors.primary-active}` — #0d1218): The press state on primary botões.

### Superfície
- **Canvas** (`{colors.canvas}` — #ffffff): The default page surface; the floor of every editorial body.
- **Superfície Soft** (`{colors.surface-soft}` — #f8fafc): Tabbed feature cards and the featured pricing tier.
- **Superfície Strong** (`{colors.surface-strong}` — #e0e2e6): The light gray "Start building with Airtable" CTA banner near the rodapé.
- **Superfície Dark** (`{colors.surface-dark}` — #181d26): The dark navy CTA cards used mid-page (for example "The path to 10× every person in your organization").
- **Superfície Dark Elevated** (`{colors.surface-dark-elevated}` — #1d1f25): The articles-page hero base behind the rainbow-stripe overlay.
...
- **Corpo** (`{colors.body}` — #333840): The default running-text color.
- **Suave** (`{colors.muted}` — #41454d): Footer links, breadcrumbs, captions.
- **Border Strong** (`{colors.border-strong}` — #9297a0): The 1px outline color on disabled secondary botões.
- **On Primary / On Dark** (`{colors.on-primary}` — #ffffff): The text color on primary botões and superfície escuras.
...
- **Cream** (`{colors.signature-cream}` — #f5e9d4): The cream callout band ("The path to 10× every person in your organization") — a soft beige surface holding dark type and interface do produto fragments.
- **Peach** (`{colors.signature-peach}` — #fcab79), **Mint** (`{colors.signature-mint}` — #a8d8c4), **Yellow** (`{colors.signature-yellow}` — #f4d35e), **Mustard** (`{colors.signature-mustard}` — #d9a441): Demo-card surfaces that carry small interface do produto fragments inside the multi-card grid sections.
...
| `{typography.body-md}` | 14px | 400 | 1.25 | 0 | Body copy, rodapé links, top-nav items |
| `{typography.caption}` | 14px | 500 | 1.35 | 0.16px | Light captions and meta text |
| `{typography.legal}` | 13.12px | 600 | 1.2 | 0 | Cookie/legal CTA botões |
...
### Principles
The Haas system prefers weight 400 for display sizes — a 40px h1 is **not** bold. Visual emphasis is delegated to size, color contrast, and the signature surface cards. Where the system does want weight, it pivots to 500 (sub-titles, botões, article titles), never 600 or 700 in the editorial body. The only true bold (600) lives in `{typography.legal}` — a sign that boldness is reserved for terms-of-service surfaces, not marketing.
...
- **Gutters:** `{spacing.lg}` (24px) between cards in 3-up grids; `{spacing.md}` (16px) inside denser logo strips and rodapé column gutters.
...
| Level | Treatment | Use |
|---|---|---|
| Flat | No shadow, no border | Body sections, top nav, rodapé |
| Soft hairline | 1px `{colors.hairline}` border | Inputs, sub-nav rails, comparison-table dividers, secondary botões |
| Button rest | Soft drop with subtle blue-tinted glow at low alpha | Primary CTA botões (the blue tint is a holdover from the link color and reads as a faint accent under the dark button) |
| Button focus | Outer 2px blue ring at higher alpha | Keyboard focus state on primary botões |
...
The elevation philosophy is **color-block first, shadow second**. Shadows are minimal; depth is delegated to the contrast between canvas branco and signature surface cards. There is no soft-glow / atmospheric-shadow / heavy-elevation language anywhere in the marketing system.
...
- **Photography-as-depth** in the demo-card grid: every card carries a real interface do produto screenshot or mockup, contributing depth through legible artifact density rather than decorative effects.
...
| Token | Value | Use |
|---|---|---|
| `{rounded.xs}` | 2px | Cookie-consent and legal CTA botões — system-required surfaces |
| `{rounded.sm}` | 6px | Text inputs, small inline botões |
| `{rounded.md}` | 10px | Secondary content cards, article cards, cream callouts |
| `{rounded.lg}` | 12px | Primary CTA botões, signature surface cards, tabbed feature cards |
| `{rounded.pill}` | 9999px | Pricing-page CTA botões (sub-system only) |
| `{rounded.full}` | 9999px / 50% | Circular icon botões, avatar surfaces |
...
**`button-primary`** — The signature CTA principal. Background `{colors.primary}` (near-black), text `{colors.on-primary}`, type `{typography.button}`, padding 16px × 24px, rounded `{rounded.lg}` (12px). This is the "Get started for free" / "Sign up for free" button visible on every hero. It reads as confident and final — not decorative — which is why the system uses it sparingly (one per viewport).
...
**`button-secondary-on-dark`** — Same shape as `{component.button-secondary}` but used on signature coral / forest / superfície escuras. Background `{colors.canvas}`, text `{colors.ink}` — the white button stays white over superfície escuras because the system never inverts to a translucent on-dark style on the marketing site.
...
**`cream-callout-card`** — Beige callout cards (`{colors.signature-cream}`). Rounded `{rounded.md}` (10px), internal padding `{spacing.lg}` (24px). Carry interface do produto fragments or stat callouts — softer than the dark/coral signature cards but still a deliberate brand surface.

**`demo-grid-card`** — Used in multi-card grids that punctuate every page. Background `{colors.canvas}` or one of the demo-grid surfaces (`{colors.signature-peach}`, `{colors.signature-mint}`, `{colors.signature-yellow}`, `{colors.signature-mustard}`), rounded `{rounded.md}` (10px), internal padding `{spacing.md}` (16px). Each card frames a interface do produto fragment. Card heights vary deliberately to dodge a uniform "spec sheet" feel.
...
**`rodapé`** — Light surface (`{colors.canvas}`), 6-column link list at desktop covering Platform / Solutions / Resources / Learn / Company sub-trees. Vertical padding `{spacing.section}` divided across upper link block and lower legal row. Type `{typography.body-md}`.

**`cta-band-light`** — The light gray "Start building with Airtable" CTA strip near the rodapé. Background `{colors.surface-strong}` (#e0e2e6), text `{colors.ink}`, rounded `{rounded.lg}` (12px), padding `{spacing.xxl}` (48px). Carries an h2 in `{typography.display-md}` and a `{component.button-primary}`.
...
### Do
- Keep `{component.button-primary}` near-black. The brand's CTA principal is `{colors.primary}`, not the link blue. Mixing them up turns a confident hero into a confused one.
...
|---|---|---|
| Mobile | < 768px | Single-column body; top nav collapses to hamburger; demo-grid drops to 1-up; signature cards stay full-bleed; logo strip wraps to 2 rows; rodapé collapses to single-column |
...
- The CSS variable `--theme_button-background-primary: #1b61c9` exists at `:root` but is not used as the CTA principal color anywhere on the marketing site. It maps to the link/info color role instead. Documented here so future extractions don't re-trip over the misleading variable name.
